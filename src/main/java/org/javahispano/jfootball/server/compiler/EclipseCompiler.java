...
/**
 * This class copiles a set of java sources using the Eclipse EJC compiler. The
 * nice thing about this approach is that it compiles the sources from memory so
 * no disk access and it works correctly under Java Web Start.
 */
public class EclipseCompiler {
	/**
	 * The class loader which will be used to load the classes once they have
	 * been compiled
	 */
	private EclipseClassLoader _classloader = new EclipseClassLoader(this.getClass().getClassLoader());
	 ...
	 * @throws Exception
	 *             if an error occurs during compilation that would stop us
	 *             loading and using the compiled class (i.e. compiler warnings,
	...
	public void compile(final Map<String, String> sources) throws Exception {
		...
		// is returned by the name environment when one of the classes given in
		// the
		...
		class CompilationUnit implements ICompilationUnit {
			...
			public char[] getContents() {
				return sources.get(_className).toCharArray();
			}
			 ...
			 * compilation unit.
			 * 
			 * @return the unqualified name of the class (i.e. no package
			 *         information)
			 */
			public char[] getMainTypeName() {
				int dot = _className.lastIndexOf('.');
				if (dot > 0)
					return _className.substring(dot + 1).toCharArray();
				return _className.toCharArray();
			}

			/**
			 * Returns the package name for the class defined by this
			 * compilation unit. For example, if this unit defines
			 * java.lang.String, ["java".toCharArray(), "lang".toCharArray()]
			 * would be returned.
			 * 
		...
		final INameEnvironment env = new INameEnvironment() {
			 ...
			 * "lang", "String"] becomes "java.lang.String") and search using
			 * that.
			 */
			...
			/**
			 * Tries to find the class or source file defined by the given type
			 * name. We construct a string from the compound name (e.g.
			 * "String", ["java", "lang"] becomes "java.lang.String") and search
			 * using that.
			 */
			...
			private NameEnvironmentAnswer findType(String className) {
				try {
					...
					/**
					 * otherwise, try and load the class from the normal
					 * classloader.
					 **/
					...
					if (is != null) {
						...
						// Create a writer we can use to fill a byte array with
						// the class
						// bytes read from the resource
						ByteArrayOutputStream baos = new ByteArrayOutputStream(buf.length);

						// the count of how many bytes we have read this
						// iteration
						int count;
						while ((count = is.read(buf, 0, buf.length)) > 0) {
							// read in bytes from the resource and write them
							// into memory
							baos.write(buf, 0, count);
						}

						// make sure we have fully read/storred all the bytes of
						// the class
						baos.flush();

						// Store the bytes we have read in along with the class
						// name
						ClassFileReader classFileReader = new ClassFileReader(baos.toByteArray(),
			...
			/**
			 * Is the requested name a package? We assume yes if it's not a
			 * class.
			 */
			private boolean isPackage(String name) {
				// if it is the name of a class we are compiling then it can't
				// be a package
			...
			public boolean isPackage(char[][] parentPackageName, char[] packageName) {
				...
				if (Character.isUpperCase(str.charAt(0)) && !isPackage(result))
					return false;
		...
		// A store for all the compilation options (thing of this as a souped up
		// version of the
		// command line arguments you would normally pass to javac)
		final Map<String, String> settings = new HashMap<String, String>();

		// Make sure that we access both the source and line numbers to help
		// with reporting errors
		settings.put(CompilerOptions.OPTION_LineNumberAttribute, CompilerOptions.GENERATE);
		settings.put(CompilerOptions.OPTION_SourceFileAttribute, CompilerOptions.GENERATE);

		// ignore deprecation, unused imports, missing serial version UIDs, and
		// unused local
		// variables - otherwise almost every Saxon rule would generate
		// warnings...
		...
		// source and target - Runes requires 1.5 so that is what we will
		// specify
		...
		// CompilerRequestor defines what to do with the result of a
		// compilation.
		final ICompilerRequestor requestor = new ICompilerRequestor() {
			public void acceptResult(CompilationResult result) {
				...
				if (result.hasProblems()) {
					...
					for (IProblem problem : result.getProblems()) {
						...
						if (problem.isError()) {
							// if the problem is an error (something that will
							// cause
							// compilation to fail) -- this means that any
							// compiler
				...
				if (!errors) {
					...
					// into the classloader so that they can be instantiated
					// later
			 ...
			 * 
			 * @param problem
			 *            the problem we want to store
			 */
			private void addProblem(IProblem problem) {
				// get the name of the file in which the compilation error has
				// occured
				...
				if (problemsForName == null) {
					...
					// then create a list to store them in add it to the
					// problems map
		...
		// create the compiler
		Compiler compiler = new Compiler(env, policy, new CompilerOptions(settings), requestor,
		...
		if (!problems.isEmpty()) {
			// if we have seen an error then the compile will have failed so
			// tell
			...
			for (Map.Entry<String, List<IProblem>> prob : problems.entrySet()) {
				...
				for (IProblem problem : probsForName) {
					...
					// display Error or Warning (we only ever display error as
					// the warnings are being ignored
					// but I've left this in to remind me of this so if I ever
					// put the warnings back into the
					...
					System.err.println(
							problem.getMessage() + " at line " + problem.getSourceLineNumber() + " in " + name);
				}
			...
			// we saw an error which means compilation will have failed so throw
			// an exception
	...
	/**
	 * A very simple class loader that allows us to hook into the standard class
	 * loader being used and define new classes from byte arrays.
	 * 
	...
	private static final class EclipseClassLoader extends ClassLoader {
		...
		/**
		 * Create a new class loader which simply delegates to the class loader
		 * of the current Thread context.
		 */
		public EclipseClassLoader(ClassLoader parent) {
		 ...
		 * @param bytes
		 *            the compiled bytes of the class
		 * @return an instance of the class
		 */
		protected Class defineClass(String name, byte[] bytes) {
			// define the class within the current class loader. We have to do
			// it via
			// this simple class loader as defineClass(String, byte[], int, int)
			// is protected
		...
		public void addClass(String className, byte[] data) {
			if (byteStreams.containsKey(className)) {
				System.err.println("duplicate defintion of class/resource " + className + ". It will be ignored");
			} else {
	...
	public Class compile(String name, String source) throws Exception {
		...
		Class c = Class.forName(name, false, _classloader);
	...
	/*
	 * public static void main(String args[]) throws Exception { EclipseCompiler
	 * compiler = new EclipseCompiler();
	 * 
	 * String HELLO_WORLD_CLASS_NAME = "HelloWorldTest"; String
	 * HELLO_WORLD_SOURCE =
	 * "import org.eclipse.jdt.internal.compiler.Compiler;\n\n" +
	 ...
	 * 
	 * Class compiledClass = compiler.compile(HELLO_WORLD_CLASS_NAME,
	 * HELLO_WORLD_SOURCE);
	 * 
	 * if (compiledClass==null) { System.out.println("Unable to compile file");
	 * return; }
	 * 
	 * Method m = compiledClass.getMethod("main",String[].class); m.invoke(null,
	 * new Object[]{null}); }
	 */
}
