 ...
 */
public class EclipseCompiler implements CompilerInterface
{
    private Class compiledClass;
    private SGNode compiledInstance;
    public void evaluate(String code) throws CompileError {
        String className = EclipseMemoryCompiler.getClassName(code);
        try {
            compiledClass = compile(className, code);
            _classloader = new EclipseClassLoader();
            compiledInstance = (SGNode) compiledClass.newInstance();
        } catch (Exception e) {
    ...
    public Timeline getTimeline() {
        return null; // only needed in beanshell
    }
	...
	*/
	public void compile(final Map<String,String> sources) throws Exception
	{
	    ...
	    if(!problems.isEmpty())
	    {
            ...
            CompileError compilerError = null;
	    	for (Map.Entry<String,List<IProblem>> prob : problems.entrySet())
	    	{
	    		...
	    		{
                    compilerError = new CompileError(compilerError, problem.getMessage()+" - Line "+problem.getSourceLineNumber(), problem.getSourceLineNumber());
                }
	    	}
